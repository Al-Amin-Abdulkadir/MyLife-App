...
def calorie_goal_submit(
    ...
    try:
        ...
        return templates.TemplateResponse(
            "fitness/calories.html",
            {"request": request, "current_user": current_user, "daily_goal": goal, "error": str(exc)},
            status_code=400,
        )
    return RedirectResponse(url="/fitness/dashboard", status_code=status.HTTP_303_SEE_OTHER)
...
def calorie_goal_day_page(
    ...
    try:
        ...
        return templates.TemplateResponse(
            ...
            {"request": request, "current_user": current_user, "target_date": target_date, "calorie_summary": None, "error": str(exc)},
            status_code=400,
        )
    return templates.TemplateResponse(
        "fitness/calorie_day.html",
        {"request": request, "current_user": current_user, "target_date": target_date, "calorie_summary": calorie_summary, "error": None},
    )
    ...
    try:
        ...
        return templates.TemplateResponse(
            "fitness/create_meal.html",
            {
                ...
                "current_user": current_user,
                "meal": {
                    "meal_name": meal_name, "meal_type": meal_type,
                    "calorie_in_meal": calorie_in_meal, "completion_date": completion_date,
                    "protein": protein, "carbs": carbs, "fats": fats, "notes": notes,
                },
                "error": str(exc),
            },
            status_code=400,
        )
    return RedirectResponse(url="/fitness/meals/list", status_code=status.HTTP_303_SEE_OTHER)
...
def show_meals_page(
    ...
    meals = MealTracker(db).view_meal(current_user)
    return templates.TemplateResponse(
        "fitness/meals_list.html",
        {"request": request, "current_user": current_user, "meals": meals},
    )
...
def edit_meal_page(
    ...
    meal = MealTracker(db).find_meal(current_user, meal_id)
    if not meal:
        return RedirectResponse(url="/fitness/meals/list", status_code=status.HTTP_303_SEE_OTHER)
    return templates.TemplateResponse(
        "fitness/edit_meal.html",
        {"request": request, "current_user": current_user, "meal": meal, "error": None},
    )
    ...
    try:
        ...
        return templates.TemplateResponse(
            "fitness/edit_meal.html",
            {
                ...
                "current_user": current_user,
                "meal": {
                    "id": meal_id, "meal_name": meal_name, "meal_type": meal_type,
                    "calories": calorie_in_meal, "protein": protein, "carbs": carbs,
                    "fats": fats, "time_of_log": time_of_log, "completion_date": completion_date,
                    "notes": notes,
                },
                "error": str(exc),
            },
...
def delete_meal(
    ...
    MealTracker(db).delete_meal(current_user, meal_id)
    return RedirectResponse(url="/fitness/meals/list", status_code=status.HTTP_303_SEE_OTHER)
...
def create_meal_plan_submit(
    ...
    return RedirectResponse(url="/fitness/meal-plans/list", status_code=status.HTTP_303_SEE_OTHER)


@router.get("/meal-plans/list", response_class=HTMLResponse)
def meal_plan_list_page(
    request: Request,
    current_user=Depends(require_user),
    db: Session = Depends(get_db),
):
    meal_plans = MealPlanService(db).list_meal_plans(current_user)
    return templates.TemplateResponse(
        "fitness/meal_plans_list.html",
        {"request": request, "current_user": current_user, "meal_plans": meal_plans},
    )


@router.post("/meal-plans/{identifier}/start")
def start_meal_plan_day(
    identifier: str,
    current_user=Depends(require_user),
    db: Session = Depends(get_db),
):
    try:
        MealPlanService(db).start_meal_plan_day(current_user, identifier)
    except ValueError:
        pass
    return RedirectResponse(url="/fitness/meal-plans/list", status_code=status.HTTP_303_SEE_OTHER)
...
def nutrition_summary_page(
    ...
    nutrition_service = NutritionSummaryService(db)
    try:
        summary = nutrition_service.calculate_daily_nutrition_summary(current_user, calculation_date)
    except ValueError as exc:
        return templates.TemplateResponse(
            "fitness/nutrition_summary.html",
            {"request": request, "current_user": current_user, "calculation_date": calculation_date, "summary": None, "error": str(exc)},
            status_code=400,
        )
    return templates.TemplateResponse(
        "fitness/nutrition_summary.html",
...
def create_workout_submit(
    ...
    workout_service = WorkoutSessionService(db)
    try:
        workout_blocks = _parse_json_list(workout_blocks_json, "workout_blocks")
        workout_service.log_session(
            current_user=current_user,
        ...
        return templates.TemplateResponse(
            "fitness/create_workout.html",
            {
                "request": request,
                "current_user": current_user,
                "workout": {"session_name": session_name, "workout_blocks_json": workout_blocks_json, "notes": notes},
                "error": str(exc),
            },
            status_code=400,
        )
    return RedirectResponse(url="/fitness/workouts/list", status_code=status.HTTP_303_SEE_OTHER)
...
def workout_list_page(
    ...
    workouts = Workout_Repository(db).list_workout_entries(current_user)
    return templates.TemplateResponse(
        "fitness/workouts_list.html",
        {"request": request, "current_user": current_user, "workouts": workouts},
    )
...
def create_routine_submit(
    ...
    try:
        ...
        routine_service.create_routine(current_user=current_user, name=name, days=days)
    except ValueError as exc:
        return templates.TemplateResponse(
            "fitness/create_routine.html",
            {
                "request": request,
                "current_user": current_user,
                "routine": {"name": name, "days_json": days_json},
                "error": str(exc),
            },
            status_code=400,
        )
    return RedirectResponse(url="/fitness/routines/list", status_code=status.HTTP_303_SEE_OTHER)


@router.get("/routines/list", response_class=HTMLResponse)
def routine_list_page(
    request: Request,
    current_user=Depends(require_user),
    db: Session = Depends(get_db),
):
    routines = RoutineService(db).list_routines(current_user)
    return templates.TemplateResponse(
        "fitness/routines_list.html",
        {"request": request, "current_user": current_user, "routines": routines},
    )
...
def routine_detail_page(
    ...
    routine = RoutineService(db).get_routine(current_user, identifier)
    if not routine:
        return RedirectResponse(url="/fitness/routines/list", status_code=status.HTTP_303_SEE_OTHER)
    return templates.TemplateResponse(
        "fitness/routine_detail.html",
        {"request": request, "current_user": current_user, "routine": routine},
    )
...
def session_from_routine_submit(
    ...
    routine_service = RoutineService(db)
    try:
        routine_service.build_session_from_routine(
            current_user=current_user,
            day_name=day_name,
        ...
        return templates.TemplateResponse(
            "fitness/routines_list.html",
            {"request": request, "current_user": current_user, "routines": routines, "error": str(exc)},
            status_code=400,
        )
    return RedirectResponse(url="/fitness/workouts/list", status_code=status.HTTP_303_SEE_OTHER)
