...
@calculate_time
def brew_tea(tea_type : str, steep_time : int) -> str:
    print(f"Brewing {tea_type}tea")
    time.sleep(steep_time)
    print("Tea is ready")
    return f"drink tea in {datetime.now() + timedelta(minutes=30)}"

@calculate_time
def make_matcha(make_time : int) -> str :
    print("Making matcha")
    time.sleep(make_time)
    print("Matcha is ready")
    return f"drink tea in {datetime.now() + timedelta(minutes=30)} minutes"
